...
I wrote code in `org.mattrr78.sparsefetchdemo.EmployeeService.initializeSparseClass()` to create a sparse class.
I then store this sparse class's constructor into the service's field to be used later to create new instances (sparse instances).
...
the best solution.  It's fast, low memory overhead, don't have to rely on working with maps (no typecasting required),
