...
TL;DR:  JPA query -> `List<Map<String, Object>>` -> New object instances from a Byte Buddy generated class.
...
Mockito uses Byte Buddy, so I can pull off what I just talked about.  It's already available as a library too me since
Hibernate uses it.

So I've written code in `org.mattrr78.sparsefetchdemo.EmployeeService.initializeSparseClass()` to create a sparse class.
I then store this sparse class's constructor into the service's field to be used to create new instances (sparse instances).
...
## Conclusion
Using a class generated by ByteBuddy to take Map result of a JPA query and store into a new sparse instances seems like
the best solution.  It's fast, lower memory overhead, don't have to rely on working with maps (no typecasting required),
